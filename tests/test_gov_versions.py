...
def test_set_version_rejects_invalid_format(tmp_path: Path) -> None:
    ...
    r = _run(repo_root, ["--set-version", "1"])
    assert r.returncode != 0
    assert "invalid Version: format" in (r.stderr + r.stdout)

    r2 = _run(repo_root, ["--set-version", "v1.2.3"])
    assert r2.returncode != 0
    assert "invalid Version: format" in (r2.stderr + r.stdout)
...
def test_future_law_included(tmp_path: Path) -> None:
    repo_root = tmp_path
    ...
    (repo_root / "scripts" / "gov_versions.py").write_text(
        src_tool.read_text(encoding="utf-8"),
        encoding="utf-8",
    )
