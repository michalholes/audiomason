...
from audiomason.import_flow import (
    BookGroup,
    _choose_books,
    _choose_source,
)
...
def test_disable_choose_books_skips_prompt(monkeypatch, tmp_path: Path):
    ...
    b1 = BookGroup(
        label="One", group_root=tmp_path, stage_root=tmp_path, rel_path=Path("."), m4a_hint=None
    )
    b2 = BookGroup(
        label="Two", group_root=tmp_path, stage_root=tmp_path, rel_path=Path("."), m4a_hint=None
    )
