        ...
        decisions.append(
            {
                ...
                "clean_stage": (
                    bool(dec.get("clean_stage")) if isinstance(dec, dict) and ("clean_stage" in dec) else None
                ),
            }
        )
...
def _detect_books(stage_src: Path) -> list[BookGroup]:
    ...
    for rel_s, root in sorted(pairs, key=lambda t: t[0].casefold()):
        ...
        books.append(
            BookGroup(label=label, group_root=root, stage_root=stage_src, m4a_hint=_find_first_m4a(root), rel_path=rp)
        )
...
def _collect_audio_files(group_root: Path) -> list[Path]:
    mp3s = sorted(
        [p for p in group_root.iterdir() if p.is_file() and p.suffix.lower() == ".mp3"], key=lambda p: p.name.casefold()
    )
    m4as = sorted(
        [p for p in group_root.iterdir() if p.is_file() and p.suffix.lower() == ".m4a"], key=lambda p: p.name.casefold()
    )
    opuses = sorted(
    ...
    def _process_one_source(
        ...
        try:
            ...
            def _step_clean_stage() -> None:
                ...
                default_clean = (
                    bool(dec.get("clean_stage")) if isinstance(dec, dict) and ("clean_stage" in dec) else False
                )
            ...
            if reuse_stage and use_manifest_answers and default_author:
                ...
                if _ol_enabled(cfg):
                    ...
                    if getattr(state, "DEBUG", False):
                        out(
                            f"[ol] author result: ok={getattr(ar, 'ok', None)} "
                            f"status={getattr(ar, 'status', None)!r} "
            ...
            for bi, b in enumerate(picked_books, 1):
                ...
                if reuse_stage and use_manifest_answers and default_title:
                    ...
                    if _ol_enabled(cfg):
                        ...
                        if getattr(state, "DEBUG", False):
                            out(
                                f"[ol] book result: ok={getattr(br, 'ok', None)} "
                                f"status={getattr(br, 'status', None)!r} "
                ...
                default_cover_mode = (
                    str(bm.get(b.label, {}).get("cover_mode") or "").strip()
                    if (reuse_stage and use_manifest_answers and isinstance(bm, dict))
                    else ""
                )
                default_cover_src = (
                    str(bm.get(b.label, {}).get("cover_src") or "").strip()
                    if (reuse_stage and use_manifest_answers and isinstance(bm, dict))
                    else ""
                )
