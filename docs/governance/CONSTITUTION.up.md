...
# CONSULTANT-LEVEL GOVERNANCE (MANDATORY)

All CONSULTANT chats MUST follow the rules defined in:

docs/CONSULTANT_CHARTER.md

CONSULTANT_CHARTER.md is AUTHORITATIVE for:
- consultant role behavior
- proposal handling in consultant chats
- audit and governance discussions

In case of conflict, this contract always takes precedence.

### CONSULTANT CHARTER CHANGE WINDOW (EXPLICITLY ALLOWED)

In a CONSULTANT chat, the assistant MAY:
- propose changes to docs/CONSULTANT_CHARTER.md as PENDING
- after explicit ACCEPTED by the Project Owner:
  - generate an updated docs/CONSULTANT_CHARTER.md for download
  - provide git commands for commit and push

Restrictions:
- the commit MUST modify ONLY docs/CONSULTANT_CHARTER.md
- no other files may be changed
- no implementation, patch scripts, or runner commands are allowed

This change window does NOT alter the chat role.
---

# GOVERNANCE ENFORCEMENT CLARIFICATIONS (HC-1..HC-5)

These clarifications are normative and binding.

## HC-1. Governance Watchdog Hook (MANDATORY)

Governance violations MUST be actively identified and flagged by CONSULTANT chats.
Sanctions and enforcement mechanisms remain defined exclusively by this contract.

## HC-2. Language-Level Violations (MANDATORY)

Language that creates implicit assistant authority or inevitability constitutes a governance violation,
regardless of intent.

## HC-3. No-Precedent Rule (MANDATORY)

Previous chats, historical behavior, or past approvals MUST NOT be used as precedent
or implicit authorization.

## HC-4. Mandatory Stop (MANDATORY)

When a governance violation or role ambiguity is detected, a Mandatory Stop MUST be applied
until the Project Owner makes an explicit decision.

Mandatory Stop is NOT an implementation FAIL-FAST. It is a governance pause.

## HC-5. Violation Typing Table (INFORMATIONAL)

This table is informational only. It does not modify enforcement rules.

| Violation type | Example | Typical response |
|---|---|---|
| Language | implicit authority / inevitability | flag + correction (and Mandatory Stop if needed) |
| Role | PM decides; Implementation plans; mixed chat purpose | Mandatory Stop + reroute |
| Process | bypass decision window; implement PENDING proposal | FAIL-FAST or HARD STOP per this contract |
...
This CONSULTANT chat MUST follow:
- docs/CONSULTANT_CHARTER.md
```
